...
<head>
    <title>Renewable Energy based Electric Vehicle Charging Station</title>
    <style>
        body {
        ...
        .solar {
            background-image: linear-gradient(to bottom, #ffd300, #ffce00);
        }
...
const String AUTHORIZED_CARD_UID = "93 26 71 17"; // Replace with your card's UID
...
void handleData() {
  float solarCurrent = (analogRead(32) * 5.0) / 10023.0; // Assuming current sensor calibration for 5V
  float solarVoltage = (analogRead(34) * 2.5) / 1023.0; // Assuming voltage sensor calibration for 5V
  float windCurrent = (analogRead(35) * 5.0) / 10023.0; // Assuming current sensor calibration for 5V
  float windVoltage = (analogRead(36) * 2.5) / 1023.0; // Assuming voltage sensor calibration for 5V
